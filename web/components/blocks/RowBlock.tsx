...
import type { RowBlock } from '@ons-mierloos-theater/shared/schemas/blocks';

export function RowBlockDisplay({ block }: { block: RowBlock }) {
  return (
...
export function RowBlockEdit({ block, onChange }: RowBlockEditProps) {
  return (
...
export function RowBlockComponent({ block, mode, onChange }: RowBlockComponentProps) {
  if (mode === 'display') {
    return <RowBlockDisplay block={block} />;
  }
  return <RowBlockEdit block={block} onChange={onChange!} />;
}
